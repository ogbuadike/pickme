...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  Future<void> _startRideMarket() async {
    ...
    _marketSub = _rideMarketService.stream(origin: safePickup(), destination: safeDrop(), originProvider: safePickup, destinationProvider: safeDrop, userIdProvider: () => _prefs.getString('user_id') ?? _user?['id']?.toString() ?? '', pollInterval: const Duration(seconds: 2)).listen((snap) {
      if (!mounted) return;
      _offers = snap.offers;
    ...
    return Scaffold(
      ...
      bottomNavigationBar: (!_marketOpen && _tripPhase == TripPhase.browsing)
        ...
        onTap: (i) {
          ...
          switch (i) {
            case 0: break;
            case 1: Navigator.pushNamed(context, AppRoutes.rideHistory); break;
            case 2: break;
            case 3: break;
