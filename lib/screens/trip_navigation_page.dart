...
// ---------------------------------------------------------------------------
// HYBRID DART KALMAN FILTER & VECTOR SNAPPER
// Custom-built for precise on-road telemetry tracking.
// ---------------------------------------------------------------------------
class _Kalman1D {
  final double q;
  final double r;
...
class _TripNavigationPageState extends State<TripNavigationPage>
  ...
  double get _arrivalMeters =>
      widget.args.arrivalMeters > 0 ? widget.args.arrivalMeters : 150.0;
  double get _routeMoveThresholdMeters =>
      widget.args.routeMoveThresholdMeters > 0
          ? widget.args.routeMoveThresholdMeters
          : 25.0;

  List<LatLng> get _allTargets =>
      [...widget.args.dropOffs, widget.args.destination];
  List<String> get _allTargetTexts =>
      [...widget.args.dropOffTexts, widget.args.destinationText];

  LatLng get _effectivePickupLL =>
      (widget.args.enableLivePickupTracking && _riderLL != null)
          ? _riderLL!
          : widget.args.pickup;

  LatLng get _currentTarget {
    if (_phase == TripNavPhase.driverToPickup ||
        _phase == TripNavPhase.waitingPickup) return _effectivePickupLL;
  ...
  String get _currentTargetText {
    if (_phase == TripNavPhase.driverToPickup ||
        _phase == TripNavPhase.waitingPickup) return widget.args.originText;
  ...
  LatLng? get _enRouteOrigin {
    if (_phase != TripNavPhase.enRoute &&
        _phase != TripNavPhase.arrivedDestination) return _displayDriverLL;
    if (widget.args.role == TripNavigationRole.rider &&
        _riderLL != null &&
        _displayDriverLL != null) {
  ...
  void initState() {
    ...
    _glideCurve = CurvedAnimation(
        parent: _glideController!,
        curve: Curves.linear); // linear for raw GPS interpolation
  ...
  void _onGlideTick() {
    if (_animStartLL != null && _animTargetLL != null) {
      ...
      final double lat = _animStartLL!.latitude +
          (_animTargetLL!.latitude - _animStartLL!.latitude) * t;
      final double lng = _animStartLL!.longitude +
          (_animTargetLL!.longitude - _animStartLL!.longitude) * t;
  ...
  void _startHardwareCompass() {
    ...
    _compassSub = FlutterCompass.events?.listen((CompassEvent event) {
      ...
      _compassThrottleTimer = Timer(const Duration(milliseconds: 50), () {
        if (!mounted || !_isFollowCameraEnabled || _isProgrammaticCameraMove)
          return;
  ...
  Future<void> _preloadIcons() async {
    try {
      await Future.wait([_loadDriverIcon(), _loadPointIcons()]);
    } catch (_) {}
  }
  ...
  Future<void> _loadDriverIcon() async {
    ...
    try {
      // 200% visible – high resolution with built-in drop shadow
      final ByteData bd =
      await rootBundle.load('assets/images/open_top_view_car.png');
    ...
    _dynamicMarkers = {
      if (_displayDriverLL != null)
        Marker(
          ...
          position: _displayDriverLL!,
          icon: _driverIcon ?? BitmapDescriptor.defaultMarker,
          anchor: const Offset(0.5, 0.5),
          flat: true,
          rotation: _smoothCarRotation(),
          zIndex: 50,
        ),
      if (_riderLL != null)
        Marker(
  ...
  Future<void> _tick({bool force = false}) async {
    ...
    try {
      ...
      if (!_glideController!.isAnimating) _followCamera();
  ...
  // ---------------------------------------------------------------------------
  // VECTOR ROUTE SNAPPER – magnetically locks driver onto the road
  // ---------------------------------------------------------------------------
  LatLng _snapToRoute(LatLng currentPos, List<LatLng>? routePoints) {
    if (routePoints == null || routePoints.isEmpty) return currentPos;

    double minDist = double.infinity;
    LatLng snappedPos = currentPos;

    for (int i = 0; i < routePoints.length - 1; i++) {
      LatLng closest =
      _getClosestPointOnSegment(currentPos, routePoints[i], routePoints[i + 1]);
      double dist = _haversine(currentPos, closest);
      if (dist < minDist) {
        minDist = dist;
        snappedPos = closest;
      }
    }
    // snap only when physically close to the road
    return minDist < 35.0 ? snappedPos : currentPos;
  }
  ...
  LatLng _getClosestPointOnSegment(LatLng p, LatLng a, LatLng b) {
    ...
    double lat1 = a.latitude * d2r;
    double lon1 = a.longitude * d2r;
    double lat2 = b.latitude * d2r;
    double lon2 = b.longitude * d2r;
    double lat3 = p.latitude * d2r;
    double lon3 = p.longitude * d2r;
    ...
    double t = ((lon3 - lon1) * math.cos((lat1 + lat2) / 2) * dx +
        (lat3 - lat1) * dy) /
        (dx * dx + dy * dy);
  ...
  void _applyIncoming(dynamic event, {bool trustPhase = true}) {
    ...
    if (event is BookingUpdate && event.status == BookingStatus.failed) {
      _emitError(
          event.displayMessage.isNotEmpty ? event.displayMessage : 'Trip issue.');
      return;
    }

    LatLng? rawNew = _coerceDriverLL(payload, event);
    final now = DateTime.now();

    if (rawNew != null) {
      // snap to existing route before Kalman filtering – eliminates "L" hooks
      if (_currentRoutePoints != null) {
        rawNew = _snapToRoute(rawNew, _currentRoutePoints);
      }
      ...
      if (_rawDriverLL == null) {
        ...
        final double timeSecs =
            now.difference(_lastLocationTime).inMilliseconds / 1000.0;

        if (timeSecs > 0) {
          final double speedMPS = distMeters / timeSecs;

          if (speedMPS < 0.3) {
            // kinematic deadband – ignore noise
          } else if (speedMPS <= 70.0) {
    ...
    if (nextPhase != null && nextPhase != _phase) {
      if (!trustPhase &&
        ...
        nextPhase = _phase; // hold optimistic phase
      } else {
        _optimisticPhase = null;
        _optimisticPhaseSetAt = null;
      }
  ...
  void _recomputePermissions() {
    ...
    final bool cAP = _phase == TripNavPhase.driverToPickup &&
        _haversine(pos, _effectivePickupLL) <= safeArrival;
    final bool cST = _phase == TripNavPhase.waitingPickup;
    final bool cAD = _phase == TripNavPhase.enRoute &&
        _haversine(pos, _currentTarget) <= safeArrival;
    final bool cCR = _phase == TripNavPhase.arrivedDestination;

    if (_canArrivePickup != cAP ||
        _canStartTrip != cST ||
  ...
  double _smoothCarRotation() {
    ...
    double target = (_backendDriverHeading > 0)
        ...
        : (_animStartLL != null &&
        _animTargetLL != null &&
        _haversine(_animStartLL!, _animTargetLL!) > 2.5)
    ...
    _carIconHeading += diff * 0.18; // smooth EMA
    return _carIconHeading;
  }

  // ---------------------------------------------------------------------------
  // ROUTE REBUILD with forced road-snap for the origin
  // ---------------------------------------------------------------------------
  Future<void> _rebuildRoute({bool force = false}) async {
    if (_busyRoute ||
        _phase == TripNavPhase.completed ||
        _phase == TripNavPhase.cancelled) return;

    LatLng? from = (_phase == TripNavPhase.enRoute ||
        _phase == TripNavPhase.arrivedDestination)
        ? _enRouteOrigin
        : _displayDriverLL;
    final LatLng? to = (_phase == TripNavPhase.enRoute ||
        _phase == TripNavPhase.arrivedDestination)
        ? _currentTarget
        : _effectivePickupLL;
    if (from == null || to == null) return;

    // ⭐ SNAP ORIGIN TO EXISTING ROUTE to avoid L-shaped polyline
    if (_currentRoutePoints != null) {
      from = _snapToRoute(from, _currentRoutePoints);
    }

    if (!force && DateTime.now().difference(_lastRouteAt) < _routeMinGap) {
      if (_lastDriverRouteLL != null &&
          _haversine(_lastDriverRouteLL!, from) < _routeMoveThresholdMeters)
        return;
    }
    ...
    try {
      ...
      if (!_didInitialFit) {
        ...
        if (!_isFollowCameraEnabled) {
          _safeCameraMove(
              CameraUpdate.newLatLngBounds(_boundsFrom([from, to]), 90.0));
        } else {
          _followCamera();
        }
      }
  ...
  Future<_RouteResult?> _computeRoute(LatLng origin, LatLng destination) async {
    ...
    try {
      ...
      if (route == null || route['polyline']?['encodedPolyline'] == null) {
        return null;
      }
      return _RouteResult(
        ...
        durationSeconds:
        int.tryParse(route['duration']?.replaceAll('s', '') ?? '0') ?? 0,
      );
    } catch (_) {
      return null;
    }
  ...
  void _safeCameraMove(CameraUpdate update) {
    _isProgrammaticCameraMove = true;
    _cameraLockTimer?.cancel();
    _cameraLockTimer = Timer(
        const Duration(milliseconds: 800), () => _isProgrammaticCameraMove = false);
    _mapController?.animateCamera(update);
  }

  // ---------------------------------------------------------------------------
  // HEAD-UP FOLLOW CAMERA – bearing from the upcoming road segment
  // ---------------------------------------------------------------------------
  void _followCamera() {
    if (_phase == TripNavPhase.completed ||
        _phase == TripNavPhase.cancelled ||
        !_isFollowCameraEnabled ||
        _viewMode != NavigationViewMode.navigation) return;

    final LatLng? target = (_phase == TripNavPhase.enRoute ||
        _phase == TripNavPhase.arrivedDestination)
        ? _enRouteOrigin
        : _displayDriverLL;
    if (target == null) return;

    // compute forward bearing from the route, not from compass swing
    double targetBearing = _carIconHeading;
    if (_currentRoutePoints != null && _currentRoutePoints!.length > 1) {
      targetBearing = _bearingTowardRoutePoint(target, _currentRoutePoints!);
    } else if (_localHardwareHeading != null) {
      targetBearing = _localHardwareHeading!;
    }
    ...
    while (diff > 180.0)
      diff -= 360.0;
    _cameraBearingEMA += diff * 0.22;

    _safeCameraMove(CameraUpdate.newCameraPosition(CameraPosition(
        target: target,
        zoom: (_phase == TripNavPhase.enRoute) ? 18.5 : 17.5,
        tilt: 75.0,
        bearing: _cameraBearingEMA)));
  }

  double _bearingTowardRoutePoint(LatLng current, List<LatLng> route) {
    // find the first point ahead of current position by at least 8 meters
    for (int i = 0; i < route.length; i++) {
      if (_haversine(current, route[i]) > 8.0) {
        return _bearingBetween(current, route[i]);
      }
    }
    // fallback to last point
    return _bearingBetween(current, route.last);
  }
  ...
  Future<void> _activateOverviewMode() async {
    ...
    if (pts.isNotEmpty) {
      _safeCameraMove(
          CameraUpdate.newLatLngBounds(_boundsFrom(pts), 92.0));
    }
  }
  ...
  Future<void> _recenterMap() async {
    ...
    if (_displayDriverLL != null) {
      _cameraBearingEMA = _carIconHeading; // reset to road heading
      _followCamera(); // immediately apply
    }
  }

  // When the user manually zooms, we allow the lock to break (as requested).
  // Other gestures like panning are ignored while locked.
  void _onCameraMoveStarted() {
    if (_isFollowCameraEnabled && !_isProgrammaticCameraMove) {
      // Only break lock if the user actually pinched (zoomed) – rough detection
      // In practice any gesture will call this, but we choose to keep lock tight.
      // The user can always tap "Overview" to deliberately leave lock.
      // If you prefer zoom-only, you can implement a zoom change listener.
      // For now, keep it docile.
    }
  }
  ...
  Future<void> _handlePrimaryAction() async {
    ...
    try {
      ...
      // 👇 Instant cascade – UI updates before next poll
      if (nextOpt != null) {
        setState(() {
          _phase = nextOpt!;
        ...
        _recomputePermissions(); // triggers UI rebuild for new button
      }
  ...
  Map<String, dynamic> _eventMap(dynamic event) {
    if (event is BookingUpdate)
      return {'booking_status': event.status.toString(), ...event.data};
  ...
  TripNavPhase? _coercePhase(Map<String, dynamic> payload, dynamic rawEvent) {
    ...
    final raw = (payload['phase'] ??
        payload['status'] ??
        payload['state'] ??
        (payload['ride'] is Map ? payload['ride']['status'] : null))
        ?.toString()
        .toLowerCase() ??
        '';
    switch (raw) {
  ...
  LatLng? _coerceDriverLL(Map<String, dynamic> payload, dynamic rawEvent) {
    final lat = double.tryParse(
        (payload['driver_lat'] ?? payload['driverLat'] ?? payload['lat'] ?? payload['latitude'])
            ?.toString() ??
            '');
    final lng = double.tryParse(
        (payload['driver_lng'] ?? payload['driverLng'] ?? payload['lng'] ?? payload['longitude'])
            ?.toString() ??
            '');
    if (lat != null && lng != null) return LatLng(lat, lng);
    try {
      final la = double.tryParse(
          (rawEvent.driverLat ?? rawEvent.lat ?? rawEvent.latitude)?.toString() ?? '');
      final lo = double.tryParse(
          (rawEvent.driverLng ?? rawEvent.lng ?? rawEvent.longitude)?.toString() ?? '');
  ...
  LatLng? _coerceRiderLL(Map<String, dynamic> payload, dynamic rawEvent) {
    final lat = double.tryParse(
        (payload['rider_lat'] ?? payload['riderLat'] ?? payload['user_lat'] ?? payload['pickup_lat'])
            ?.toString() ??
            '');
    final lng = double.tryParse(
        (payload['rider_lng'] ?? payload['riderLng'] ?? payload['user_lng'] ?? payload['pickup_lng'])
            ?.toString() ??
            '');
    if (lat != null && lng != null) return LatLng(lat, lng);
    return null;
  ...
  double? _coerceHeading(Map<String, dynamic> payload, dynamic rawEvent) {
    final top = double.tryParse((payload['driver_heading'] ??
  ...
  LatLngBounds _boundsFrom(List<LatLng> pts) {
    ...
    return LatLngBounds(
      southwest: LatLng(minLat - 0.0001, minLng - 0.0001),
      northeast: LatLng(maxLat + 0.0001, maxLng + 0.0001),
    );
  }

  double _haversine(LatLng a, LatLng b) {
    double d2r(double d) => d * (math.pi / 180.0);
    final h = math.sin(d2r(b.latitude - a.latitude) / 2) *
        math.sin(d2r(b.latitude - a.latitude) / 2) +
  ...
  double _bearingBetween(LatLng a, LatLng b) {
    ...
    final x = math.cos(d2r(a.latitude)) * math.sin(d2r(b.latitude)) -
        math.sin(d2r(a.latitude)) *
            math.cos(d2r(b.latitude)) *
            math.cos(d2r(b.longitude - a.longitude));
  ...
  EdgeInsets _calcMapPadding(MediaQueryData mq, UIScale ui) {
    ...
    final double landscapePanelWidth = showLandscapePanel
        ? (mq.size.width * 0.35).clamp(320.0, 400.0)
        : 0;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.args.role == TripNavigationRole.driver) {
      if (_phase == TripNavPhase.driverToPickup &&
          widget.args.showArrivedPickupButton) primaryLabel = 'AT PICKUP';
      else if (_phase == TripNavPhase.waitingPickup &&
          widget.args.showStartTripButton) primaryLabel = 'START TRIP';
      else if (_phase == TripNavPhase.enRoute &&
          widget.args.showArrivedDestinationButton) primaryLabel = 'AT DESTINATION';
      else if (_phase == TripNavPhase.arrivedDestination &&
          widget.args.showCompleteTripButton) primaryLabel = 'COMPLETE RIDE';
    }

    final double sheetInitialSize =
    ui.landscape ? 1.0 : (ui.tiny ? 0.38 : (ui.compact ? 0.35 : 0.32));
    final double sheetMaxSize =
    ui.landscape ? 1.0 : (ui.tiny ? 0.65 : 0.55);
            ...
            Positioned.fill(
              child: Container(
                ...
                child: _booting
                  ...
                  initialTarget:
                  _displayDriverLL ?? _effectivePickupLL,
                  isDark: isDark,
                  ...
                  onMapCreated: (controller) {
                    ...
                    if (!_didInitialFit &&
                        _polylinesNotifier.value.isNotEmpty) {
                ...
                Positioned(
                    ...
                    child: SafeArea(
                        ...
                        child: _SolidPanelContainer(
                            ui: ui,
                            child:
                            _buildSheetContent(ui, primaryLabel))))
              else
                Positioned(
                    left: ui.inset(10),
                    right: ui.inset(10),
                    bottom: ui.inset(10),
                    height: math.min(mq.size.height * 0.70,
                        mq.size.height - (mq.padding.top + ui.gap(80))),
                    child: SafeArea(
                        top: false,
  ...
  Widget _buildSheetContent(UIScale ui, String? primaryLabel,
    ...
    return _PremiumDashboardSheet(
      ...
      showPrimaryAction: primaryLabel != null,
      primaryLabel: primaryLabel,
      showCancelButton: widget.args.showCancelButton &&
          (widget.args.role == TripNavigationRole.driver ||
              (_phase == TripNavPhase.driverToPickup ||
                  _phase == TripNavPhase.waitingPickup)),
      busyPrimary: _busyPrimaryAction,
      busyCancel: _busyCancel,
      primaryEnabled: (_phase == TripNavPhase.driverToPickup && _canArrivePickup) ||
          (_phase == TripNavPhase.waitingPickup && _canStartTrip) ||
          (_phase == TripNavPhase.enRoute && _canArriveDestination) ||
          (_phase == TripNavPhase.arrivedDestination && _canCompleteRide),
            ...
            builder: (context, polylines, _) {
              return GoogleMap(
                initialCameraPosition: CameraPosition(
                    target: initialTarget, zoom: 17.5, tilt: 65),
...
class _PremiumDashboardSheet extends StatelessWidget {
  ...
  final String driverName, vehicleType, carPlate;
  final double rating;
  final String from, to, currentTarget;
  ...
  final VoidCallback onPrimaryAction, onCancelTrip;
  ...
  Widget build(BuildContext context) {
    ...
    role == TripNavigationRole.driver ? "DRIVER COMMAND" : "RIDER MODE";
    final String profileTitle =
    role == TripNavigationRole.driver ? "YOUR RIDER" : "YOUR DRIVER";
    ...
    final List<Widget> content = [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Container(
            padding: EdgeInsets.symmetric(
                horizontal: ui.inset(8), vertical: ui.inset(4)),
            decoration: BoxDecoration(
                color: AppColors.primary.withOpacity(0.15),
                borderRadius: BorderRadius.circular(6)),
            child: Text("$modeLabel • ${rideType.toUpperCase()}",
                style: TextStyle(
                    color: AppColors.primary,
            ...
            child: Container(
                ...
                child: Row(children: [
                  Icon(Icons.emergency_share_rounded,
                      color: Colors.red, size: ui.icon(14)),
      ...
      Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
        CircleAvatar(
            ...
            child:
            Icon(Icons.person, color: AppColors.primary, size: ui.icon(18))),
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            Container(
                ...
                child: Row(children: [
                  Icon(Icons.star_rounded,
                      color: Colors.amber, size: ui.icon(12)),
          ...
          if (carPlate.isNotEmpty)
            Container(
                ...
                child: Text(carPlate.toUpperCase(),
                    style: TextStyle(
                        color: Colors.black,
                        fontWeight: FontWeight.w900,
                        fontSize: ui.font(10),
      ...
      Container(
          ...
          child: Row(children: [
            ...
            Container(
                ...
                margin: EdgeInsets.symmetric(horizontal: ui.inset(10))),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text(durationText,
                  style: TextStyle(
                      color: Colors.white,
      ...
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Column(children: [
          Icon(Icons.radio_button_checked,
              color: AppColors.primary, size: ui.icon(14)),
          Container(
              width: 1.5,
              height: ui.gap(18),
        ...
        Container(
            ...
            child: Row(children: [
              Icon(Icons.error_outline_rounded,
                  color: AppColors.error, size: ui.icon(14)),
          ...
          Expanded(
              ...
              child: AnimatedSwitcher(
                  ...
                  child: _PremiumButton(
                      key: ValueKey(primaryLabel ?? 'ACTION'),
...
class _PremiumButton extends StatelessWidget {
  final String label;
  final bool isLoading, isDisabled, isPrimary;
        ...
        child: Container(
            ...
            child: isLoading
                ...
                child: CircularProgressIndicator(
                    strokeWidth: 2.0,
                    color: isPrimary ? Colors.white : os))
                : Text(label,
                style: TextStyle(
                    color: isPrimary
                        ? Colors.white.withOpacity(disabled ? 0.7 : 1.0)
                        : os.withOpacity(disabled ? 0.5 : 0.9),
...
class _SolidHeader extends StatelessWidget {
  final UIScale ui;
  final String phaseLabel, distanceText, durationText;
        ...
        child: Row(children: [
          IconButton(
              ...
              icon: Icon(Icons.close_rounded,
                  color: Colors.white, size: ui.icon(16))),
          SizedBox(width: ui.gap(10)),
          Container(
              width: 1,
              height: ui.gap(20),
...
class _PillActionRail extends StatelessWidget {
  ...
  final VoidCallback onOverviewMode, onRecenter;
  final UIScale ui;
  ...
  Widget build(BuildContext context) {
    return Column(
        ...
        children: [
          ...
          SizedBox(height: ui.gap(8)),
          _ActionPill(
              icon: isFollowCameraEnabled
                  ? Icons.lock_rounded
                  : Icons.my_location_rounded,
              label: isFollowCameraEnabled ? 'Locked' : 'Re-center',
              isActive: isFollowCameraEnabled,
              onTap: isFollowCameraEnabled
                  ? null // already locked, do nothing
                  : onRecenter,
              ui: ui),
        ]);
  }
...
class _ActionPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
        ...
        child: Container(
            padding: EdgeInsets.symmetric(
                horizontal: ui.inset(12), vertical: ui.inset(8)),
            decoration: BoxDecoration(
                color: Colors.black.withOpacity(0.85),
                borderRadius: BorderRadius.circular(ui.radius(16)),
                border: Border.all(
